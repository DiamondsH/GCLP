...
class GCN(nn.Module):
    def __init__(self, in_feature, hidden, out_feature, dropout):
        ...
        self.conv1 = GATConv(in_feature, hidden)
        self.conv2 = GATConv(hidden, out_feature)
        self.dropout_rate = dropout
        self.sigmoid = torch.nn.Sigmoid()
...
class FCN_LP(nn.Module):
    ...
    def forward(self, data, return_mid=False):
        ...
        for i in range(len(self.gc)-1):
            ...
            x = F.relu(x)
            x = F.dropout(x, self.dropout_rate, training=self.training)
            if return_mid and i == len(self.gc) // 2 - 1:
                x_mid = x  # 记录中间层特征
        ...
        if return_mid:
            return out.squeeze(), y_hat.squeeze(), x, x_mid
