...
class LPAconv(MessagePassing):
    ...
    def forward(
        ...
        # 确保 edge_index 是 long 类型并拆分
        edge_index = edge_index.long()
        row, col = edge_index[0], edge_index[1]

        # 构建 SparseTensor（关键修复！）
        # adj_t = SparseTensor(
        #     row=row,
        #     col=col,
        #     value=edge_attr if edge_attr is not None else None,
        #     sparse_sizes=(x.size(0), x.size(0))
        # )

        # x_l = self.lin_l(x).view(-1, self.heads, self.out_channels)  # [N, H, C]
        # x_r = self.lin_r(x).view(-1, self.heads, self.out_channels)
        x_l = self.lin_l(x)  #W_l*h_i [N,H*C]
        x_r = self.lin_r(x)  #W_r*h_j

        # propagate 接收 SparseTensor
        y_out = self.propagate(edge_index, x=(x_l, x_r), label=label, edge_attr=edge_attr)

        # 注意：y_out 形状可能需要调整，取决于你的 propagate 实现
        return y_out
